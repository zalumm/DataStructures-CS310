...
public class Driver {
    ...
    public static void main(String [] args) {
        ...
        for(int i=0; i < DICTIONARY_SIZE*10; i++) {
            ...
            Integer val = dictionary.getValue(testArray[index]);
            if(val == null)
        ...
        for(int i=0; i < DICTIONARY_SIZE/10; i++) {
            int index = ( (int) (DICTIONARY_SIZE*Math.random()));
            String key = dictionary.getKey(new Integer(index));
            if(key == null)
                throw new RuntimeException(
                        "ERROR, key " + testArray[index] + " was not found");
    ...
    private static String[] getRandomStrings(int size) {
        ...
        for(int i=0; i < 10; i++)
            for(int j=0; j < 10; j+= 2 *Math.random()+1)
                for(int k=0; k < 10; k+= 2 *Math.random()+1)
                    for(int l=0; l < 26; l+= 2 *Math.random()+1)
                        for(int m=0; m < 26; m+= 2 *Math.random()+1)
                            for(int n=0; n < 26; n++) {
                                if(where >= size) break;
                                str[where++] = ""+
                                        ((char)(b[0]+n)) +
